...
import time
_gemini_last_error_time = 0
_gemini_cooldown = 60  # Wait 60s after 429 before retrying
...
def analyze_with_gemini(img_bytes, defect_library):
    global _gemini_last_error_time, _gemini_cooldown
    # Skip if still in cooldown period
    if time.time() - _gemini_last_error_time < _gemini_cooldown:
        print("[Gemini] Skipping — still in cooldown")
        return None
    try:
...
- JSON only, no other text"""
        ...
        if '429' in str(e):
            ...
            print("[Gemini] Rate limited — cooling down 60s")
        return None
